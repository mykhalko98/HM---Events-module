...
namespace Modules\Events\Widgets\EventUrl;
...
class Controller extends Widget
{
    public function handle()
    {
        ...
        $view_vars = core()->filters()->apply_filters('widget_view_vars', ['config' => $config, 'content' => $content, 'event' => $event]);

        if ($is_edit) {
            return view('widgets.events::EventUrl.edit', $view_vars);
        }

        return view('widgets.events::EventUrl.index', $view_vars);
    }
