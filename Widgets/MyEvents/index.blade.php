@if(in_array($filter_by, ['category', 'tag']))
    <h3 class="mb-3">{{__('events::lang.MY_EVENTS_TITLE')}}, {{ ['tag' => 'Tag', 'category' => 'Category'][$filter_by] }}: {{ $filter_value }}</h3>
@else
    <h3 class="mb-3">{{__('events::lang.MY_EVENTS_TITLE')}}</h3>
@endif
                ...
                <div class="card">
                    <a href="{{ $event->getUrl() }}" class="card-link">
                        ...
                        @if($event->status == 'draft')
                            <span class="badge right badge-white">{{ __('Draft') }}</span>
                        @endif
                    </a>
